...
def main():
    ...
    if args.ip:
        network_monitor = IPMonitor(print_lock=print_lock, osint_queue=osint_queue, db_path=ip_db_path, db_lock=ip_db_lock)
    if args.dns:
        dns_interceptor = DNSMonitor(print_lock=print_lock, osint_queue=osint_queue, db_path=domain_db_path, db_lock=domain_db_lock, interface_manual=args.interface_manual)
